...
var realX = 2; //m
var realY = 1; //m
var pictureX = 1920; //px
var pictureY = 1080; //px
...
function setup()
{
	...
	// Scalce Calculation
	xScaling = width / realX;
	yScaling = height / realY;
	...
	{	
		...
		testbutton.size(120, 40);	
		testbutton.position(0.02 * width, 0.8 * height);
		testbutton.mousePressed(pressedTestButton);

		changewindbutton = createButton('Windspeed');
		changewindbutton.position(0.02 * width, 0.9 * height)
		changewindbutton.size(120, 40);
...
function nextRound()
{
	...
	windspeedkmh = randomizeWindSpeed(windspeedScale);
...
function draw() {
	...
	{
		textSize(40);	
		text(leftPoints + " : " + rightPoints, 0.48 * width, 0.15 * height);

		textSize(25);
		text("Windspeed: " + windspeedkmh.toFixed(2) + " km/h", 0.44 * width, 0.2 * height);

		drawArrow(createVector(0.5 * width, 0.25 * height), createVector(0.03 * windspeedkmh * xScaling, 0), '#fafafa')
	}

	textSize(18);
	if(debug) 
	{
		text("Time: " + t.toFixed(2) + " s" 
			+ "\nDelta: " + dt.toFixed(3) + " s"
			
			+ "\n\n\nThrowing Time: " + l_ThrowTime.toFixed(2) 
			+ "\nAgular-Left: " + angVelocityLeft.toFixed(2) 
			+ "\nvx: " + vxL.toFixed(2) + " / vy: " + vyL.toFixed(2)

			+ "\n\n\nThrowing Time: " + r_ThrowTime.toFixed(2) 
			+ "\nAgular-Right: " + angVelocityRight.toFixed(2) 
			+ "\nvx: " + vxR.toFixed(2) + " / vy: " + vyR.toFixed(2)

			+"\n\n\nWindspeed: " + windspeedkmh.toFixed(2) + " km/h"

			,0.02*width, 0.06*height
		);

		text("xL: " + xL.toFixed(2) + " -- yL: " + yL.toFixed(2) 
			+ "\nxR: " + xR.toFixed(2) + " -- yR: " + yR.toFixed(2)

			+ "\n\n\nxC0: " + xC0.toFixed(2) + " -- yC0: " + yC0.toFixed(2) 
			+ "\nxC: " + xC.toFixed(2) + " -- yC: " + yC.toFixed(2)

			+ "\n\n\nxM0: " + xM0.toFixed(2) + " -- yM0: " + yM0.toFixed(2) 
...
function randomizeWindSpeed(number) {
	...
	var temp
	do temp =  Math.random() * (number + number) - number;
