...
function draw() {
	...
	*/
	...
	fill('#ff000050');
	strokeWeight(0);
	rect(-0.4 * xScaling, 0, 0.05 * xScaling, 1 * yScaling);
	rect(+0.35 * xScaling, 0, 0.05 * xScaling, 1 * yScaling);
	...
	fill('#fafafa');
	textSize(40);
	if (!customBall) text("0" + " : " + "0", 460, 100);
...
function customCollosion() {
	...
	if (t_ <= dt && !customCollosioning) {
		console.log("Hello... touching")
		dt = t_;
		customCollosioning = true;
	}

	else {
		dt = 1 / fps;
		customCollosioning = false;
	}
